...
src/
  pages/
    ...
    claim-deadlines-ending-soon.astro   # Settlements expiring within 14 days (SSR)
    class-action-settlements-database.astro # Sortable/filterable settlement table (SSR)
...
scripts/
  ...
  update_case_status.py         # Weekly metadata updater (Perplexity, dry-run supported)
...
| CollectionPage | Category hubs, brand pages, state pages, settlement database |
| ItemList | Open settlements, deadlines ending soon, settlement database |
...
/open-class-action-settlements → /settlements/{slug}, /news/{slug}, /claim-deadlines-ending-soon, /class-action-settlements-database
...
| Open settlements, deadlines ending soon, settlement database, brand pages, state pages | SSR | Dynamic data from Supabase |
...
Two GitHub Actions workflows manage content:
...
- **Update Case Status** (`update-case-status.yml`) — weekly metadata-only updater. Checks non-closed settlements against Perplexity for source-supported status changes. Updates only metadata fields (case_status, settlement_amount, claim_deadline, etc.). Never modifies article content. Supports dry-run mode via `DRY_RUN=1`.
